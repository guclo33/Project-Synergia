...
synergia_couleur_string = excel_to_string(plage_questions1_15)


#section archétype

synergia_archetype_string = excel_to_string(plage_questions16_24)


#Questions développement


synergia_section_developpement_string = excel_to_string(plage_questions_developpement)
...
#Section Comment s'adapter

adapter_prompt = """Je souhaite obtenir une section de 300 mots par couleur décrivant les stratégies de communication adaptées aux différentes couleurs (rouge, jaune, bleu, vert), teintées de la personnalité spécifique de la personne. Pour chaque couleur, crée un paragraphe qui met en avant les points suivants : 
1.	Comment la personne peut communiquer efficacement avec les personnes de cette couleur.
2.	Quels styles de leadership ou d'interaction seront les plus alignés avec cette couleur.
3.	Intégrer les forces et traits principaux de la personne pour rendre la communication plus fluide et efficace.
Exemple :
Pour s’adapter au rouge
 🗣 Communication Directe et Concise : Mylène devrait privilégier une communication claire et aller droit au but. Les rouges apprécient la directivité et l'efficacité dans les échanges. En évitant les détails superflus et en se concentrant sur l'essentiel, elle gagnera en efficacité dans ses interactions avec eux.
⚡ Prise de Décision Rapide et Assurée : Il serait bénéfique pour Mylène de montrer de l'assurance et une capacité à prendre des décisions rapidement. Les rouges estiment grandement la rapidité et la fermeté dans la prise de décision. Même si elle préfère analyser en détail, elle peut s'exercer à synthétiser ses analyses pour des décisions plus rapides.
🎯 Focus sur les Résultats : Mylène pourrait mettre en avant les objectifs et les résultats obtenus. Les rouges sont impressionnés par l'impact concret des actions. En soulignant ses accomplissements et en démontrant comment ses contributions mènent à des résultats tangibles, elle alignera ses efforts avec les attentes des rouges.
🛡 Leadership Fort et Assertif : Adopter une posture de leadership affirmée peut aider Mylène à gagner le respect des rouges, qui valorisent la force de caractère et la détermination. En montrant qu'elle peut prendre les rênes et diriger avec confiance, elle sera perçue comme une alliée forte et capable.
📊 Présentation des Idées de Manière Pragmatique : Lors de la communication de ses idées, Mylène devrait les formuler de façon pragmatique, soulignant les bénéfices et impacts directs. Les rouges apprécient les approches pratiques et les résultats immédiats, donc en mettant en avant les avantages concrets, elle pourra mieux capter leur attention.
🌪 Adaptabilité et Réactivité : Montrer une capacité à s'adapter rapidement aux changements et répondre efficacement aux demandes des rouges est crucial. Bien que Mylène préfère une planification détaillée, démontrer qu'elle peut être flexible et réactive face à l'imprévu sera un atout.
"""


context_append(travail_text, adapter_prompt)

adapter = generateur_texte(message_data, 2000)

adapter_text = adapter.choices[0].message.content
